package com.groupfour.eMovie.entity;

public class RecommendOverview {
    private int movieid;
    private int movie1;
    private int movie2;
    private int movie3;
    private int movie4;
    private int movie5;
    private int movie6;
    private int movie7;
    private int movie8;
    private int movie9;

//    public boolean existDuplicate(int id) {
//        if ()
//    }

    public int getMovieid() {
        return movieid;
    }

    public void setMovieid(int movieid) {
        this.movieid = movieid;
    }

    public int getMovie1() {
        return movie1;
    }

    public void setMovie1(int movie1) {
        this.movie1 = movie1;
    }

    public int getMovie2() {
        return movie2;
    }

    public void setMovie2(int movie2) {
        this.movie2 = movie2;
    }

    public int getMovie3() {
        return movie3;
    }

    public void setMovie3(int movie3) {
        this.movie3 = movie3;
    }

    public int getMovie4() {
        return movie4;
    }

    public void setMovie4(int movie4) {
        this.movie4 = movie4;
    }

    public int getMovie5() {
        return movie5;
    }

    public void setMovie5(int movie5) {
        this.movie5 = movie5;
    }

    public int getMovie6() {
        return movie6;
    }

    public void setMovie6(int movie6) {
        this.movie6 = movie6;
    }

    public int getMovie7() {
        return movie7;
    }

    public void setMovie7(int movie7) {
        this.movie7 = movie7;
    }

    public int getMovie8() {
        return movie8;
    }

    public void setMovie8(int movie8) {
        this.movie8 = movie8;
    }

    public int getMovie9() {
        return movie9;
    }

    public void setMovie9(int movie9) {
        this.movie9 = movie9;
    }
}
